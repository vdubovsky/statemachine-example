...
import static io.vdubovsky.statemachineexample.statemachine.configuration.pingpong.PingPongState.PING_PONG_SET_ACTION_PING_STATE;


@Component
public class PingPongStateMachineSetActionPingAction implements ActionAware<PingPongState, PingPongEvent> {
    @Override
    public PingPongState getStateEntryPoint() {
        return PING_PONG_SET_ACTION_PING_STATE;
    }
    ...
    public void execute(StateContext<PingPongState, PingPongEvent> context) {
        //Action
        context.getExtendedState().getVariables().put("action", "ping");
